...
export const config={warmup:Number(process.env.WARMUP||30),samples:Number(process.env.SAMPLES||120),repeats:Number(process.env.REPEATS||3),size:512,tests:process.env.TESTS?.split(',')||(process.argv.includes('--3d')?threeDWorkloads:workloads),mode:process.argv.includes('--frames')?'frames':'latency',profileAllocations:process.argv.includes('--allocations')};
...
export async function save(path,result) {
  ...
  for(const capture of result.captures||[]) {
    ...
    for(let y=0;y<capture.height;y++) raw.copy(png.data,y*capture.width*4,(capture.height-1-y)*capture.width*4,(capture.height-y)*capture.width*4);
